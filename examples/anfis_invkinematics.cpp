...
void MakeTrainingSets(fl::DataSet<>& data1, fl::DataSet<>& data2);

void MakeTestSets(fl::DataSet<>& data1, fl::DataSet<>& data2);

FL_unique_ptr<fl::anfis::Engine> BuildAndTrainAnfis(const fl::DataSet<>& data, std::size_t numInMFs, const std::string& algo, std::size_t maxEpochs, fl::scalar goalError, bool online);

fl::scalar TestAnfis(fl::anfis::Engine* p_anfis, const fl::DataSet<>& testSet);

fl::scalar ValidateAnfis(fl::anfis::Engine* p_anfis1, const fl::DataSet<>& testSet1, fl::anfis::Engine* p_anfis2, const fl::DataSet<>& testSet2, const std::string& algo, bool online);
...
void MakeTrainingSets(fl::DataSet<>& data1, fl::DataSet<>& data2)
{
	...
	// Creates x-y-theta1 and x-y-theta2 datasets
	// {MATLAB: data1 = [X(:) Y(:) THETA1(:)];}
	// {MATLAB: data2 = [X(:) Y(:) THETA2(:)];}
	data1 = fl::DataSet<>(numInputs, 1);
	data2 = fl::DataSet<>(numInputs, 1);
	for (std::size_t j = 0,
		 ...
		 ++j)
	{
			 ...
			 ++i)
		{
			...
			std::vector<fl::scalar> outputs(1);
			inputs[0] = X[i][j];
			inputs[1] = Y[i][j];
...
void MakeTestSets(fl::DataSet<>& data1, fl::DataSet<>& data2)
{
	...
	// {MATLAB: XY1 = [X(:) Y(:) THETA1D];}
	// {MATLAB: XY2 = [X(:) Y(:) THETA2D];}
	data1 = fl::DataSet<>(numInputs, 1);
	data2 = fl::DataSet<>(numInputs, 1);
	for (std::size_t j = 0,
...
FL_unique_ptr<fl::anfis::Engine> BuildAndTrainAnfis(const fl::DataSet<>& data, std::size_t numInMFs, const std::string& algo, std::size_t maxEpochs, fl::scalar goalError, bool online)
{
	...
	{
		fl::anfis::Jang1993HybridLearningAlgorithm* p_hybrid = new fl::anfis::Jang1993HybridLearningAlgorithm(p_anfis.get());
		if (online)
		{
	...
	{
		fl::anfis::Jang1993GradientDescentBackpropagationAlgorithm* p_gdss = new fl::anfis::Jang1993GradientDescentBackpropagationAlgorithm(p_anfis.get());
		if (online)
		{
	...
	{
		fl::anfis::GradientDescentWithMomentumBackpropagationAlgorithm* p_gdm = new fl::anfis::GradientDescentWithMomentumBackpropagationAlgorithm(p_anfis.get());
		if (online)
		{
	...
	p_trainAlgo->train(data, maxEpochs, goalError);
...
fl::scalar TestAnfis(fl::anfis::Engine* p_anfis, const fl::DataSet<>& testSet)
{
		 ...
		 ++entryIt)
	{
		...
		const fl::scalar err = THETAD.back()-THETAP.back();
		rmse += err*err;
	}
...
{
	// {MATLAB: XY = [X(:) Y(:)];}
	// {MATLAB: THETA1P = evalfis(XY, anfis1);}
	// {MATLAB: THETA2P = evalfis(XY, anfis2);}
	// {MATLAB: THETAP = [THETA1P; THETA2P];}
	// {MATLAB: save data/ikdata_check.dat THETAP;}
	...
	fl::scalar rmse = 0;
	for (fl::DataSet<>::ConstEntryIterator entryIt = testSet1.entryBegin(),
										   entryEndIt = testSet1.entryEnd();
		 entryIt != entryEndIt;
		 ++entryIt)
	{
		const fl::DataSetEntry<> entry = *entryIt;

		const std::vector<fl::scalar> anfisOut = p_anfis1->eval(entry.inputBegin(), entry.inputEnd());
	...
	for (fl::DataSet<>::ConstEntryIterator entryIt = testSet2.entryBegin(),
										   entryEndIt = testSet2.entryEnd();
		 entryIt != entryEndIt;
		 ++entryIt)
	{
		const fl::DataSetEntry<> entry = *entryIt;

		const std::vector<fl::scalar> anfisOut = p_anfis2->eval(entry.inputBegin(), entry.inputEnd());
	...
	rmse = std::sqrt(rmse/(testSet1.size()+testSet2.size()));
...
int main(int argc, char* argv[])
{
	...
	std::cout << "Making training sets..." << std::endl;

	fl::DataSet<> data1; // x-y-theta1 dataset
	fl::DataSet<> data2; // x-y-theta2 dataset

	MakeTrainingSets(data1, data2);

	//std::cout << "Data1: "; PrintDataSet(std::cout, data1); std::cout << std::endl;
	//std::cout << "Data2: "; PrintDataSet(std::cout, data2); std::cout << std::endl;

	// Building the ANFIS models

	std::cout << "Building & Training ANFIS networks..." << std::endl;

	// - Builds and trains the first ANFIS network
	...
	p_anfis1 = BuildAndTrainAnfis(data1, 7, algo, maxEpochs, goalError, online);
	p_anfis1->setName("ANFIS1");
	std::cout << "Built & Trained the first ANFIS network: " << std::endl
			  << p_anfis1->toString() << std::endl;
	std::cout << "Building & Training the second ANFIS network..." << std::endl;
	// - Builds and trains the second ANFIS network
	//  MATLAB: anfis2 = anfis(data2, 6, 150, [0,0,0,0])
	FL_unique_ptr<fl::anfis::Engine> p_anfis2;
	p_anfis2 = BuildAndTrainAnfis(data2, 6, algo, maxEpochs, goalError, online);
	p_anfis2->setName("ANFIS2");
	std::cout << "Built & Trained the second ANFIS network: " << std::endl
			  << p_anfis2->toString() << std::endl;


	// Validating the ANFIS models

	std::cout << "Making test sets..." << std::endl;

	fl::DataSet<> testData1;
	fl::DataSet<> testData2;
	MakeTestSets(testData1, testData2);

	//std::cout << "Test Data1: "; PrintDataSet(std::cout, testData1); std::cout << std::endl;
	//std::cout << "Test Data2: "; PrintDataSet(std::cout, testData2); std::cout << std::endl;

	std::cout << "Testing ANFIS networks..." << std::endl;

	fl::scalar rmse = 0;

	std::cout << "Testing the first ANFIS network..." << std::endl;
	rmse = TestAnfis(p_anfis1.get(), testData1);
	std::cout << "Tested -> RMSE: " << rmse << std::endl;

	std::cout << "Testing the second ANFIS network..." << std::endl;
	rmse = TestAnfis(p_anfis2.get(), testData2);
	std::cout << "Tested -> RMSE: " << rmse << std::endl;

	std::cout << "Validating ANFIS networks..." << std::endl;

	rmse = ValidateAnfis(p_anfis1.get(), testData1, p_anfis2.get(), testData2, algo, online);

	std::cout << "Validated -> RMSE: " << rmse << std::endl;
}
